...
#         text = font.render(f"{i+1}. Fitness: {enemy.chromosome[59]:.1f}", True, text_color)
...
#     # Display group dynamics statistics
#     y_offset += 20
#     group_stats_header = font.render("Group Dynamics", True, YELLOW)
#     screen.blit(group_stats_header, (SCREEN_WIDTH // 2 - group_stats_header.get_width() // 2, y_offset))
#     y_offset += 25
    
#     # Calculate average group preferences
#     avg_group_size = sum(enemy.chromosome[54] for enemy in population) / len(population)
#     avg_proximity = sum(enemy.chromosome[55] for enemy in population) / len(population)
    
#     # Count different formation roles
#     roles = ["Edge", "Middle", "Leader"]
#     role_counts = [0, 0, 0]
#     for enemy in population:
#         role_value = enemy.chromosome[56]
#         if role_value <= 0.3:
#             role_counts[0] += 1  # Edge
#         elif role_value <= 0.7:
#             role_counts[1] += 1  # Middle
#         else:
#             role_counts[2] += 1  # Leader
    
#     # Count different formation patterns
#     pattern_names = ["Line", "Circle", "Triangle", "Grid"]
#     pattern_counts = [0, 0, 0, 0]
#     for enemy in population:
#         pattern = int(enemy.chromosome[57])
#         pattern_counts[pattern] += 1
    
#     # Display group statistics
#     group_size_text = font.render(f"Avg Group Size Preference: {avg_group_size:.1f} enemies", True, WHITE)
#     screen.blit(group_size_text, (SCREEN_WIDTH // 2 - group_size_text.get_width() // 2, y_offset))
#     y_offset += 20
    
#     proximity_text = font.render(f"Avg Optimal Proximity: {avg_proximity:.1f} pixels", True, WHITE)
#     screen.blit(proximity_text, (SCREEN_WIDTH // 2 - proximity_text.get_width() // 2, y_offset))
#     y_offset += 20
    
#     # Role distribution
#     roles_text = font.render("Formation Roles: ", True, WHITE)
#     screen.blit(roles_text, (SCREEN_WIDTH // 2 - 200, y_offset))
#     for i, (role, count) in enumerate(zip(roles, role_counts)):
#         percentage = (count / len(population)) * 100
#         role_text = font.render(f"{role}: {percentage:.1f}%", True, WHITE)
#         screen.blit(role_text, (SCREEN_WIDTH // 2 - 50 + i * 120, y_offset))
#     y_offset += 20
    
#     # Pattern distribution
#     patterns_text = font.render("Formation Patterns: ", True, WHITE)
#     screen.blit(patterns_text, (SCREEN_WIDTH // 2 - 200, y_offset))
#     patterns_y = y_offset
#     for i, (pattern, count) in enumerate(zip(pattern_names, pattern_counts)):
#         percentage = (count / len(population)) * 100
#         if i < 2:
#             pattern_text = font.render(f"{pattern}: {percentage:.1f}%", True, WHITE)
#             screen.blit(pattern_text, (SCREEN_WIDTH // 2 - 50 + i * 120, patterns_y))
#         else:
#             pattern_text = font.render(f"{pattern}: {percentage:.1f}%", True, WHITE)
#             screen.blit(pattern_text, (SCREEN_WIDTH // 2 - 50 + (i-2) * 120, patterns_y + 20))
#     y_offset += 40
...
#     group_text = font.render("Group Behavior:", True, YELLOW)
#     screen.blit(group_text, (box_x + 10, y_offset))
#     y_offset += 20
    
#     # Group size preference
#     size_pref = enemy.chromosome[54]
#     size_text = font.render(f"Size Pref: {size_pref:.1f}", True, WHITE)
#     screen.blit(size_text, (box_x + 10, y_offset))
#     y_offset += 16
    
#     # Optimal proximity
#     proximity = enemy.chromosome[55]
#     prox_text = font.render(f"Proximity: {proximity:.1f}px", True, WHITE)
#     screen.blit(prox_text, (box_x + 10, y_offset))
#     y_offset += 16
    
#     # Formation role
#     role_value = enemy.chromosome[56]
#     role_name = "Leader" if role_value > 0.7 else "Middle" if role_value > 0.3 else "Edge"
#     role_text = font.render(f"Role: {role_name} ({role_value:.2f})", True, WHITE)
#     screen.blit(role_text, (box_x + 10, y_offset))
#     y_offset += 16
    
#     # Formation pattern
#     pattern_value = int(enemy.chromosome[57])
#     pattern_names = ["Line", "Circle", "Triangle", "Grid"]
#     pattern_text = font.render(f"Pattern: {pattern_names[pattern_value]}", True, WHITE)
#     screen.blit(pattern_text, (box_x + 10, y_offset))
#     y_offset += 16
    
#     # Group synchronization
#     sync_value = enemy.chromosome[58]
#     sync_text = font.render(f"Sync: {sync_value:.2f}", True, WHITE)
#     screen.blit(sync_text, (box_x + 10, y_offset))
#     screen.blit(bullet_text, (box_x + 10, y_offset))
#     y_offset += 20
